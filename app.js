...
let category='dance',duration=60,userName='',evaluationMode='camera',voiceSamplingId=null;let karaokeVideoId='';let karaokeReady=false;let karaokeLibrary=[];let karaokeSelectedItem=null;let imitationType='airguitar';let categoryChosen=false;const DEFAULT_VISUAL_IMAGE='assets/imagenes/xpresia_inicio.jpg';
const profiles={
...
function buildResultImage(r){
  ...
  if(userName){cx.fillStyle='#e0c3ff';cx.font='900 46px Arial';cx.fillText(userName,w/2,285);}
...
function updateKaraokeFolderStatus(message){
  ...
  // La interfaz de Karaoke usa únicamente la biblioteca oficial de Xpresia.
  // No se muestra al usuario ningún selector de carpeta, enlace o biblioteca externa.
}
...
activation.addEventListener('click',enterXpresia,{passive:false});
const panels=['mainMenu','dancePanel','singPanel','readingCategoryPanel','actingPanel','auraPanel','imitationPanel','evaluationPanel','learningPanel','contactPanel','collabPanel','competitionPanel','musicPanel'];
...
document.querySelectorAll('.skillCard,.secondarySkills button[data-category]').forEach(btn=>btn.onclick=()=>{if(evalRunning)return;const cat=btn.dataset.category;const panelMap={dance:'dancePanel',sing:'singPanel',reading:'readingCategoryPanel',acting:'actingPanel',aura:'auraPanel',imitation:'imitationPanel'};category=cat;categoryChosen=true;evaluationMode='camera';resetScore();updateDescription();updateTextConfigUI();updateEvaluationModeUI();openPanel(panelMap[cat]||'evaluationPanel');hideGuideText();});
document.querySelectorAll('[data-skill-return]').forEach(btn=>btn.onclick=returnToMain);
document.querySelectorAll('.skillOptionCard').forEach(btn=>btn.onclick=()=>{if(evalRunning)return;category=btn.dataset.category||category;categoryChosen=true;const action=btn.dataset.action||'eval';evaluationMode=btn.dataset.mode||'camera';updateDescription();resetScore();updateTextConfigUI();updateEvaluationModeUI();if(action==='music'){openMusicPanel();return}if(action==='karaoke'){openPanel('evaluationPanel');return}openPanel('evaluationPanel');hideGuideText();setMicrophoneEnabled(true).catch(e=>console.warn('Micrófono:',e));});document.getElementById('learnAccess')?.addEventListener('click',()=>openPanel('learningPanel'));document.getElementById('contactAccess')?.addEventListener('click',()=>openPanel('contactPanel'));document.getElementById('collabAccess')?.addEventListener('click',()=>openPanel('collabPanel'));document.getElementById('competitionAccess')?.addEventListener('click',()=>openPanel('competitionPanel'));
document.getElementById('mainClose')?.addEventListener('click',closePanels);document.getElementById('evaluationClose')?.addEventListener('click',returnToMain);document.getElementById('learningClose')?.addEventListener('click',returnToMain);document.getElementById('contactClose')?.addEventListener('click',returnToMain);document.getElementById('collabClose')?.addEventListener('click',returnToMain);document.getElementById('competitionClose')?.addEventListener('click',returnToMain);
